...
class ZoneMinder extends EventEmitter {
    ...
    auth(callback) {
        ...
        that._get2('/api/host/login.json', that.refresh_token ? '' : loginJSON, (e, json, r) => {
            var cookies;
            if (!r) {
                that.emit('errror', e);
                that.isAuth = false;
            ...
            if (json.access_token) {
                ...
                // time in Seconds
                that.access_token_expires = json.access_token_expires;
                that.refresh_token_expires = json.refresh_token_expires;

                setTimeout(that.auth.bind(that), (that.access_token_expires - 60) * 1000);
