...
const appKeyIntervall = 600000 // 10 Minuten
...
class ZoneMinder extends EventEmitter {
    constructor(options, adapter) {
        ...
        this.auth();
        setInterval(this.auth.bind(this), appKeyIntervall)
    }
    ...
    _get2(path, post, cb) {
        ...
        if(that.authkey !== null && that._cookies === null){
            options.uri += '?&'+ that.authkey;
        }
        else if(that.access_token !== null  && that._cookies === null){
            options.uri += '?&token='+ that.access_token
            post = "";
        }
