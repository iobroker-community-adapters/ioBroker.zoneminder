...
### Zoneminder Adapter Specific Context
- **Adapter Name:** zoneminder
- **Primary Function:** Integration with Zoneminder security camera management system (https://zoneminder.com/)
- **Key Dependencies:** @iobroker/adapter-core, mqtt, websocket, request-promise (for API communication)
- **Configuration Requirements:** Host URL, username/password authentication, device and monitor intervals
- **Main Features:**
  - Connect to Zoneminder server via HTTP API
  - Monitor camera/device status and alerts 
  - Support for zmEventNotification for real-time alert information
  - Websocket connection for live monitoring
  - Camera URL generation for image/video access
  - Multiple authentication methods (hash-based and direct)

### Zoneminder Integration Patterns
- Use request-promise for HTTP API calls to Zoneminder server
- Implement websocket connection for real-time event monitoring
- Handle authentication via login API endpoint and session management
- Process monitor configurations and camera metadata from Zoneminder
- Parse event notifications and alert data
- Manage polling intervals for device discovery vs. monitor checking
...
- Mock Zoneminder server responses for offline testing
- Test authentication methods (hash-based vs direct)
- Validate monitor detection and event processing
- Test websocket connection handling and reconnection
- Verify proper cleanup of intervals and connections
- Test camera URL generation with different authentication modes
- Handle network timeouts and connection failures in tests